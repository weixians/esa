...
def concatenate_recurrent_states(split_recurrent_states):
    ...
    if all(s is None for s in split_recurrent_states):
        ...
        if isinstance(non_none_s, torch.Tensor):
            new_ss = [s if s is not None else torch.zeros_like(non_none_s) for s in split_recurrent_states]
            return torch.stack(new_ss, dim=1)
        elif isinstance(non_none_s, np.ndarray):
            new_ss = [s if s is not None else np.zeros_like(non_none_s) for s in split_recurrent_states]
            return np.stack(new_ss, axis=1)
        elif isinstance(non_none_s, tuple):
            return tuple(
                concatenate_recurrent_states([s[i] if s is not None else None for s in split_recurrent_states])
...
def pack_one_step_batch_as_sequences(xs):
    if isinstance(xs, tuple):
        ...
        return nn.utils.rnn.pack_sequence([xs])
...
def pack_sequences_recursive(sequences):
    ...
    if isinstance(first_seq, tuple):
        return tuple(pack_sequences_recursive([seq[i] for seq in sequences]) for i in range(len(first_seq)))
    return sequences
