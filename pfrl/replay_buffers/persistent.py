...
class PersistentReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, dirname, capacity, *, ancestor=None, logger=None, distributed=False, group=None):
        super().__init__(capacity)

        if not distributed:
            self.memory = PersistentRandomAccessQueue(dirname, capacity, ancestor=ancestor, logger=logger)
        else:
            try:
...
class PersistentEpisodicReplayBuffer(EpisodicReplayBuffer):
    ...
    def __init__(self, dirname, capacity, *, ancestor=None, logger=None, distributed=False, group=None):
        ...
        if not distributed:
            self.memory = PersistentRandomAccessQueue(self.memory_dir, capacity, ancestor=ancestor, logger=logger)
