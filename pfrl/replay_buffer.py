...
class AbstractReplayBuffer(object, metaclass=ABCMeta):
    ...
    @abstractmethod
    def append(
        self, state, action, reward, next_state=None, next_action=None, is_state_terminal=False, env_id=0, **kwargs
    ):
...
def batch_experiences(experiences, device, phi, gamma, batch_states=batch_states):
    ...
    batch_exp = {
        ...
        "action": torch.as_tensor(np.array([elem[0]["action"] for elem in experiences]), device=device),
    ...
    if all(elem[-1]["next_action"] is not None for elem in experiences):
        batch_exp["next_action"] = torch.as_tensor([elem[-1]["next_action"] for elem in experiences], device=device)
    return batch_exp
...
def batch_recurrent_experiences(experiences, device, phi, gamma, batch_states=batch_states):
    ...
    batch_exp = {
        "state": [batch_states([transition["state"] for transition in ep], device, phi) for ep in experiences],
        "action": torch.as_tensor([transition["action"] for transition in flat_transitions], device=device),
        "reward": torch.as_tensor(
        ...
        "next_state": [
            batch_states([transition["next_state"] for transition in ep], device, phi) for ep in experiences
        ],
        "is_state_terminal": torch.as_tensor(
        ...
        "discount": torch.full((len(flat_transitions),), gamma, dtype=torch.float, device=device),
    ...
    def update_if_necessary(self, iteration):
        ...
        for _ in range(self.n_times_update):
            if self.episodic_update:
                episodes = self.replay_buffer.sample_episodes(self.batchsize, self.episodic_update_len)
