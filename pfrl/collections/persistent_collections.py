...
class PersistentRandomAccessQueue(object):
    ...
    def _load_meta(self, ancestor, maxlen):
        ...
        if self.fs.exists(self.meta_file):
            ...
            assert self.meta["comm_size"] == self.comm_size, "Reloading same basedir requires same comm.size"
    ...
    def _append(self, value):
        if self.tail.is_full():
            self.tail = _ChunkWriter(self.datadir, self.gen, self.chunk_size, self.fs, do_pickle=True)
            if self.logger:
    ...
    def _setup_fs(self, fs):
        ...
        if fs is None:
            if _chainerio_available:
                ...
                raise NotImplementedError("Internal Error: chainerio support is not yet implemented")
            else:
    ...
    def _load_ancestor(self, ancestor, num_data_needed):
        ...
        for datadir in datadirs:
            ...
            if self.logger:
                self.logger.info("%d data loaded to buffer (rank=%d)", len(rank_data), self.comm_rank)
