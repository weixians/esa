...
class DQN(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def __init__(
        ...
        batch_states: Callable[[Sequence[Any], torch.device, Callable[[Any], Any]], Any] = batch_states,
    ...
    def _setup_actor_learner_training(
        ...
        update_counter: Any,
    ) -> Tuple[torch.nn.Module, Sequence[mp.connection.Connection], Sequence[mp.connection.Connection],]:
    ...
    def update(self, experiences: List[List[Dict[str, Any]]], errors_out: Optional[list] = None) -> None:
    ...
    def update_from_episodes(self, episodes: List[List[Dict[str, Any]]], errors_out: Optional[list] = None) -> None:
    ...
    def _compute_loss(self, exp_batch: Dict[str, Any], errors_out: Optional[list] = None) -> torch.Tensor:
        ...
        try:
            ...
            while not stop_event.is_set():
                ...
                if self.recurrent:
                    assert isinstance(self.replay_buffer, AbstractEpisodicReplayBuffer)
                    with replay_buffer_lock:
                        episodes = self.replay_buffer.sample_episodes(self.minibatch_size, self.episodic_update_len)
