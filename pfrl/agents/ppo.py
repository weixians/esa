...
def _add_log_prob_and_value_to_episodes_recurrent(
    ...
    for transition, log_prob, v, next_v in zip(flat_transitions, flat_log_probs, flat_vs, flat_next_vs):
...
class PPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _update_if_dataset_is_ready(self):
        ...
        if dataset_size >= self.update_interval:
            ...
            self.explained_variance = _compute_explained_variance(list(itertools.chain.from_iterable(self.memory)))
            self.memory = []
    ...
    def _update(self, dataset):
        ...
        for batch in _yield_minibatches(dataset, minibatch_size=self.minibatch_size, num_epochs=self.epochs):
    ...
    def _update_recurrent(self, dataset):
        ...
        for _ in range(self.epochs):
            ...
            for minibatch in _yield_subset_of_sequences_with_fixed_number_of_items(dataset, self.minibatch_size):
    ...
    def _lossfun(self, entropy, vs_pred, log_probs, vs_pred_old, log_probs_old, advs, vs_teacher):
        ...
        loss = loss_policy + self.value_func_coef * loss_value_func + self.entropy_coef * loss_entropy
