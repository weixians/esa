...
def _collect_first_recurrent_states_of_policy(episodes):
    return [(ep[0]["recurrent_state"][0] if ep[0]["recurrent_state"] is not None else None) for ep in episodes]


def _collect_first_recurrent_states_of_vf(episodes):
    return [(ep[0]["recurrent_state"][1] if ep[0]["recurrent_state"] is not None else None) for ep in episodes]
...
class TRPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _update_if_dataset_is_ready(self):
        dataset_size = (
            ...
            + (0 if self.batch_last_episode is None else sum(len(episode) for episode in self.batch_last_episode))
        )
        if dataset_size >= self.update_interval:
            ...
            self.explained_variance = _compute_explained_variance(flatten_sequences_time_first(self.memory))
            self.memory = []
    ...
    def _update_vf_recurrent(self, dataset):
        ...
        for epoch in range(self.vf_epochs):
            ...
            for minibatch in _yield_subset_of_sequences_with_fixed_number_of_items(dataset, self.vf_batch_size):  # NOQA
    ...
    def _update_vf_once_recurrent(self, episodes):
        ...
        seqs_states = []
        for ep in episodes:
            states = self.batch_states([transition["state"] for transition in ep], self.device, self.phi)
            if self.obs_normalizer:
    ...
    def _update_vf(self, dataset):
        ...
        for batch in _yield_minibatches(dataset, minibatch_size=self.vf_batch_size, num_epochs=self.vf_epochs):
    ...
    def _update_policy_recurrent(self, dataset):
        ...
        with torch.no_grad():
            policy_rs = concatenate_recurrent_states(_collect_first_recurrent_states_of_policy(dataset))
    ...
    def _line_search(self, full_step, dataset, advs, action_distrib_old, gain):
        ...
        if self.recurrent:
            ...
            states = self.batch_states([transition["state"] for transition in dataset], self.device, self.phi)
            if self.obs_normalizer:
                states = self.obs_normalizer(states, update=False)
        ...
        for i in range(self.line_search_max_backtrack + 1):
            ...
            with torch.no_grad(), pfrl.utils.evaluating(self.policy):
                ...
                new_kl = torch.mean(torch.distributions.kl_divergence(action_distrib_old, new_action_distrib))
            ...
            self.logger.info("Line search coundn't find a good step size. The policy was not" " updated.")
            self.policy_step_size_record.append(0.0)
            _replace_params_data(
                policy_params,
                _split_and_reshape_to_ndarrays(flat_params, sizes=policy_params_sizes, shapes=policy_params_shapes),
            )
    ...
    def _batch_observe_eval(self, batch_obs, batch_reward, batch_done, batch_reset):
        ...
        if self.recurrent:
            ...
            indices_that_ended = [i for i, (done, reset) in enumerate(zip(batch_done, batch_reset)) if done or reset]
            if indices_that_ended:
                self.test_recurrent_states = mask_recurrent_state_at(self.test_recurrent_states, indices_that_ended)
    ...
    def _batch_observe_train(self, batch_obs, batch_reward, batch_done, batch_reset):
        ...
        if self.recurrent:
            ...
            indices_that_ended = [i for i, (done, reset) in enumerate(zip(batch_done, batch_reset)) if done or reset]
            if indices_that_ended:
                self.train_recurrent_states = mask_recurrent_state_at(self.train_recurrent_states, indices_that_ended)
