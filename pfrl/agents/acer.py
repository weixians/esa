...
def compute_policy_gradient_full_correction(action_distrib, action_distrib_mu, action_value, v, truncation_threshold):
    ...
    with torch.no_grad():
        ...
        correction_weight = torch.nn.functional.relu(1 - truncation_threshold * rho_all_inv) * action_distrib.probs[0]
...
def compute_policy_gradient_sample_correction(action_distrib, action_distrib_mu, action_value, v, truncation_threshold):
...
def compute_loss_with_kl_constraint(distrib, another_distrib, original_loss, delta):
    ...
    g = [grad[0] for grad in torch.autograd.grad([original_loss], distrib_params, retain_graph=True)]
...
class ACER(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def update_from_replay(self):
        ...
        for t, transition in enumerate(episode):
            ...
            with torch.no_grad():
                if self.recurrent:
                    ((avg_action_distrib, _, _), shared_recurrent_state,) = one_step_forward(
    ...
    def update_on_policy(self, statevar):
        ...
        if not self.disable_online_update:
            if statevar is None:
                ...
                with torch.no_grad():
                    if self.recurrent:
                        (_, _, v), _ = one_step_forward(self.model, statevar, self.train_recurrent_states)
                    else:
    ...
    def _act_train(self, obs):
        ...
        with torch.no_grad():
            if self.recurrent:
                ((avg_action_distrib, _, _), self.shared_recurrent_states,) = one_step_forward(
                    self.shared_average_model,
                    statevar,
