...
class A2C(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _flush_storage(self, obs_shape, action):
        ...
        self.states = torch.zeros(
            self.update_steps + 1, self.num_processes, *obs_shape, device=self.device, dtype=torch.float
        )
        self.actions = torch.zeros(
            self.update_steps, self.num_processes, *action_shape, device=self.device, dtype=torch.float
        )
        self.rewards = torch.zeros(self.update_steps, self.num_processes, device=self.device, dtype=torch.float)
        ...
        self.masks = torch.ones(self.update_steps, self.num_processes, device=self.device, dtype=torch.float)
    ...
    def _compute_returns(self, next_value):
        if self.use_gae:
            ...
            for i in reversed(range(self.update_steps)):
                delta = self.rewards[i] + self.gamma * self.value_preds[i + 1] * self.masks[i] - self.value_preds[i]
                gae = delta + self.gamma * self.tau * self.masks[i] * gae
                self.returns[i] = gae + self.value_preds[i]
        else:
            self.returns[-1] = next_value
            for i in reversed(range(self.update_steps)):
                self.returns[i] = self.rewards[i] + self.gamma * self.returns[i + 1] * self.masks[i]
    ...
    def update(self):
        ...
        (value_loss * self.v_loss_coef + action_loss * self.pi_loss_coef - dist_entropy * self.entropy_coeff).backward()
        ...
        # Update stats
        self.average_actor_loss += (1 - self.average_actor_loss_decay) * (float(action_loss) - self.average_actor_loss)
        self.average_value += (1 - self.average_value_decay) * (float(value_loss) - self.average_value)
    ...
    def _batch_observe_train(self, batch_obs, batch_reward, batch_done, batch_reset):
        ...
        self.rewards[self.t - self.t_start - 1] = torch.as_tensor(batch_reward, device=self.device, dtype=torch.float)
        self.states[self.t - self.t_start] = statevar
