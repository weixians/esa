...
class A3C(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def update(self, statevar):
        ...
        if statevar is None:
            ...
            with torch.no_grad(), pfrl.utils.evaluating(self.model):
                if self.recurrent:
                    (_, vout), _ = one_step_forward(self.model, statevar, self.train_recurrent_states)
                else:
    ...
    def _act_train(self, obs):
        ...
        self.average_entropy += (1 - self.average_entropy_decay) * (float(pout.entropy()) - self.average_entropy)
