...
def _apply_categorical_projection(y, y_probs, z):
    ...
    offset = torch.arange(0, batch_size * n_atoms, n_atoms, dtype=torch.int32, device=y.device)[..., None]
    # Accumulate m_l
    # Note that u - bj in the original paper is replaced with 1 - (bj - l) to
    # deal with the case when bj is an integer, i.e., l = u = bj
    z_probs.view(-1).scatter_add_(0, (l.long() + offset).view(-1), (y_probs * (1 - (bj - l))).view(-1))
    # Accumulate m_u
...
class CategoricalDQN(dqn.DQN):
    ...
    def _compute_target_values(self, exp_batch):
        ...
        Tz = (
            batch_rewards[..., None]
            + (1.0 - batch_terminal[..., None]) * torch.unsqueeze(exp_batch["discount"], 1) * z_values[None]
        )
