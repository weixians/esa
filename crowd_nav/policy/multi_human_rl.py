import torch
import numpy as np
from crowd_env.envs.utils.action import ActionRot, ActionXY
...
class MultiHumanRL(CADRL):
    def __init__(self):
        self.config = None
        self.max_neighbor_num = None
        hn_neighbor_state_dim = None
        super().__init__()
    ...
    def predict(self, state):
        ...
        if self.phase == "train" and probability < self.epsilon:
            ...
            for action in self.action_space:
                ...
                if self.query_env and self.phase != "test":
                ...
                if self.with_om:
                    if occupancy_maps is None:
                        occupancy_maps = self.build_human_neighbors(next_human_states).unsqueeze(0)
    ...
    def transform(self, state):
        ...
        if self.with_om:
            occupancy_maps = self.build_human_neighbors(state.human_states)
    ...
    def input_dim(self):
        ...
        return self.joint_state_dim + (self.hn_neighbor_state_dim if self.with_om else 0)
    ...
    def build_human_neighbors(self, human_states):
        def dist(h):
            # sort human order by decreasing distance to the robot
            return h.px**2 + h.py**2
        ...
        for human in human_states:
            ...
            for other_human in human_states:
                if other_human != human:
                    other_humans.append(
                        ObservableState(
                            other_human.px - human.px,
                            other_human.py - human.py,
                            other_human.vx - human.vx,
                            other_human.vy - human.vy,
                            other_human.radius,
                        )
                    )
            arr = []
            # 按距离从小到大排序
            other_humans = sorted(other_humans, key=dist, reverse=True)
            # 太多的话就选取前几个
            if len(other_humans) > self.max_neighbor_num:
                other_humans = other_humans[: self.max_neighbor_num]
            for h in other_humans:
                arr.extend([h.px, h.py, h.vx, h.vy, h.radius])
            # 补齐缺少的
            for i in range(self.max_neighbor_num - len(other_humans)):
