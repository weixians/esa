...
import itertools
import logging
from crowd_env.envs.policy.policy import Policy
from crowd_env.envs.utils.action import ActionRot, ActionXY
from crowd_env.envs.utils.state import ObservableState, FullState
...
class CADRL(Policy):
        ...
        if self.phase == "train" and probability < self.epsilon:
            ...
            for action in self.action_space:
                ...
                batch_next_states = torch.cat(
                    [torch.Tensor([next_self_state + next_human_state]).to(self.device) for next_human_state in ob],
                    dim=0,
                )
                # VALUE UPDATE
    ...
    def transform(self, state):
        ...
        state = self.rotate(state.unsqueeze(0))
        return state
    ...
    def rotate(self, state):
        ...
        new_state = torch.cat(
            [dg, v_pref, radius, vx, vy, px1, py1, vx1, vy1, radius1, da, radius_sum],
            dim=1,
        )
        return new_state
